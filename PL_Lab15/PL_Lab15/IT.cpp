		...
		while (ch != 'я')//странно, но как есть. Просто в конце "я" добавляется.
		{
			...
			else if (((ch == '	') && (element[0] != (char)39)) || ((ch == ' ') && (element[0] != (char)39)) || (ch == ',') || (ch == ';') || (ch == '+') || (ch == '-') || (ch == '*') || (ch == '/') || (ch == '{') || (ch == '}') || (ch == '(') || (ch == ')') || (ch == '=') || (ch == '\n'))
			{
				...
				if ((ch == ',') || (ch == ';') || (ch == '+') || (ch == '-') || (ch == '*') || (ch == '/') || (ch == '{') || (ch == '}') || (ch == '(') || (ch == ')') || (ch == '='))
				{
					LT::Entry lexEntry;
					if ((ch == '+') || (ch == '-'))
						lexEntry.lexema[0] = LEX_PLUS;
					else if ((ch == '*') || (ch == '/'))
						lexEntry.lexema[0] = LEX_STAR;
					else
...
badmetka:		std::ofstream stream;
	...
	{
		int posinLT = 0;
		while (posinLT != lextable.size)
		{
			if (lextable.table[posinLT].lexema[0] == LEX_IS)
			{
				++posinLT;
				PolNot::PolishNotation(posinLT, lextable, idtable);
			}
			else
				++posinLT;
		}

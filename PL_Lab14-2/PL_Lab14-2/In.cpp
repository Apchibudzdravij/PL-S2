...
namespace In
{
	...
	IN getin(wchar_t infile[])
	{
		...
		while(!input.eof())
		{
			...
			if ((ch == '\n') && (position >= 1) && (text[position - 1] == ','))//Убирает переход на новую строку, если прежняя закончилась запятой
			{
				ch = ' ';
				text[position] = (char)11;
				++position;
			...
			for (int i = 1; i < MAX_STRING_LENGTH; ++i)
				if ((position >= 1)&&(text[position - i] == (char)(39)))
					check = !check;
